...
export function createUsageState(dataDir, historyHours = 72) {
  ...
  let loaded = false;
  let lastTurn = null; // 最近一次完成的回合（用于客户端“本回合”提示）
  ...
  function addRecord({ t = Date.now(), sessionId = "?", model = null, usage = {} } = {}) {
    ...
    if (input + output + cacheRead > 0) {
      lastTurn = { at: t, total: input + output + cacheRead, model: m };
    }
  ...
  function currentSummary(now = Date.now()) {
    ...
    let dayObservedFrom = 0; // 今日最早观测桶（小时起点），0 = 今天还没有记录
    const byModel = new Map();
    // 24h 内按模型分桶（给“模型行展开曲线”用）
    const model24 = new Map(); // model → Map<hour,row>
    ...
    for (const b of buckets.values()) {
      if (b.h >= h24) {
        ...
        let mm = model24.get(b.m);
        if (!mm) {
          mm = new Map();
          model24.set(b.m, mm);
        }
        const row = mm.get(b.h) || { t: b.h, i: 0, o: 0, cr: 0, cw: 0, n: 0 };
        row.i += b.i; row.o += b.o; row.cr += b.cr; row.cw += b.cw; row.n += b.n;
        mm.set(b.h, row);
      }
      if (b.h >= day0) {
        if (!dayObservedFrom || b.h < dayObservedFrom) dayObservedFrom = b.h;
    ...
    for (const [model, mm] of model24) {
      const rows = [...mm.values()]
        .map((r) => ({ t: r.t, inputTokens: r.i, outputTokens: r.o, cacheReadTokens: r.cr, cacheWriteTokens: r.cw, calls: r.n }))
        .sort((a, b) => a.t - b.t);
      historyByModel.push({ model, rows });
    }
    ...
    return {
      day: {
        inputTokens: day.i,
        outputTokens: day.o,
        cacheReadTokens: day.cr,
        cacheWriteTokens: day.cw,
        calls: day.n,
        // 观测窗口标注（数据诚实）：今天第一笔观测的小时起点。
        // observedFrom > day0 表示插件并非从今天 0 点开始统计，全天可能被低估。
        observedFrom: dayObservedFrom || null,
        partialDay: dayObservedFrom > day0,
      },
      byModel: modelList,
      history,
      historyByModel,
      lastSession,
      lastTurn: lastTurn ? { at: lastTurn.at, totalTokens: lastTurn.total, model: lastTurn.model } : null,
    };
  }
