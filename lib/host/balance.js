...
/**
 * Pick the balance entry to display. The official response is a multi-currency
 * array whose order is NOT stable, so `[0]` is wrong. Heuristic (whale-widget):
 *   CNY with balance > 0 → any non-zero entry → CNY → USD → first entry.
 */
export function pickBalanceEntry(infos) {
  if (!Array.isArray(infos) || infos.length === 0) return null;
  const nonzero = (x) => x && (parseFloat(x.total_balance) || 0) > 0;
  return (
    infos.find((x) => x && x.currency === "CNY" && nonzero(x)) ||
    infos.find(nonzero) ||
...
export function createBalanceStore({ resolver, fetchImpl = globalThis.fetch, timeoutMs = REQUEST_TIMEOUT_MS }) {
  ...
  function fail(code, message, now, transient = false) {
    state.lastAttemptAt = now;
    state.error = { code, message };
    // 瞬时失败（网络/超时/5xx/限流）且此前成功过：沿用最近值并标记 stale，
    // 挂件不闪错、不丢数字（whale-widget 同款策略）；认证/解析类失败不回退。
    state.stale = transient && state.lastOkAt != null;
    failureStreak += 1;
    return { ...state };
  }
   ...
   *  In-flight 去重：并发的定时轮询与手动刷新共用同一个 promise。 */
  function poll() {
  ...
  async function doPoll() {
    ...
    try {
      ...
      if (name === "TimeoutError" || name === "AbortError") {
        return fail("timeout", `Balance request timed out after ${timeoutMs} ms.`, now, true);
      }
      return fail("network", `Balance request failed: ${err && err.message ? err.message : String(err)}`, now, true);
    }
    if (res.status === 401) {
      failureStreak = 0;
    ...
    if (res.status === 429) {
      return fail("rate-limited", "Balance API rate limited (429). Will retry with backoff.", now, true);
    }
    if (!res.ok) {
      return fail("http", `Balance API responded ${res.status}.`, now, res.status >= 500);
    }
    let body;
    try {
      body = await res.json();
    ...
    const entry = pickBalanceEntry(infos);
    if (!entry) {
